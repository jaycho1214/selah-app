...
1. `translations` - downloaded translation metadata (id, name, downloadedAt)
...
Use `sqliteTable` from drizzle-orm/sqlite-core. Use `text` for IDs (UUIDs), `integer` for timestamps (mode: timestamp). The `syncedAt` field tracks server sync status (null = not synced).
